...
Main3d.prototype = {
	...
	,_onLoad2: function() {
		html_Page.hideLoading();
		this._scene = new THREE.Scene();
		this._camera = new camera_ExCamera(60,Main3d.W / Main3d.H,10,40000);
		this._camera.near = 5;
		...
		var aLight = new THREE.AmbientLight(10066329);
		this._scene.add(aLight);
		var p = new particles_StarParticle();
		this._scene.add(p);
		this._xmasPlayer = new XmasPlayer();
		this._xmasPlayer.initObj();
	...
	,_start: function() {
		this._xmasPlayer.init(this._camera,this._renderer);
...
XmasPlayer.prototype = $extend(THREE.Object3D.prototype,{
	initObj: function() {
		this._timeline = new objects_bg_TimelineMesh();
		this._timeline.position.z = data_Params.distance / 2;
		this._timeline.position.y = 50.;
		this.add(this._timeline);
		this._xmas = new objects_bg_XmasLogo(data_DataManager.getInstance().xmas.geos[0]);
		this._xmas.scale.set(1.4,1.4,1.4);
		this._xmas.position.z = data_Params.distance - 150;
		this._xmas.position.y = 330;
		this.add(this._xmas);
		this._logo = new objects_bg_XmasLogo(data_DataManager.getInstance().nyangle.geos[0]);
		this._logo.scale.set(1.2,1.2,1.2);
		this._logo.position.z = 0;
		this._logo.position.y = 160;
		this.add(this._logo);
		this._mojis = new particles_MojiParticles();
		this._mojis.init();
		this.add(this._mojis);
	}
	,init: function(cam,renderer) {
		...
		this._audio.addEventListener("500ms",$bind(this,this._onbpm));
		this._audio.addEventListener("finish",$bind(this,this._finish));
		this._mojis.start();
		this._mojis.position.z = data_Params.distance / 2;
		this._start();
	}
	,_start: function() {
	...
	,_noteon: function(e) {
		...
		var list = [51,52,53,54,55,56,49,50];
		var _g1 = 0;
		var _g = list.length;
		while(_g1 < _g) {
			var i = _g1++;
			if(list[i] == d.id) this._mojis.flash();
		}
	}
	,_onbpm: function(e) {
		this._xmas.setColor();
	...
	,_changeAngle: function() {
		var d = camera_CamData.getAngle();
		this._angle.setCam(d);
		if(d.gyaku) {
			this._xmas.rotation.y = Math.PI;
			this._logo.rotation.y = Math.PI;
			this._mojis.rotation.y = Math.PI;
		} else {
			this._xmas.rotation.y = 0;
			this._logo.rotation.y = 0;
			this._mojis.rotation.y = 0;
		}
	}
	,_setColors: function() {
		var d = data_ColorData.getNext();
		this._renderer.setClearColor(d.bg);
		data_DataManager.getInstance().box.mat.color.copy(d.box);
		this._timeline.setColor(d.line);
		this._gen.setPaperColor(d.paper);
		this._mojis.setColor(d.moji);
	}
	,update: function() {
		if(this._gen != null) this._gen.update();
		if(this._angle != null) this._angle.update();
		if(this._mojis != null) this._mojis.update();
		if(this._timeline != null) this._timeline.update(this._audio.getTimeRatio());
	}
...
camera_CamAngle.prototype = {
	...
	,update: function() {
		this._camera.target.y = 50.;
		this._camera.target.z = sound_MyAudio.a.getTimeRatio() * data_Params.distance;
	}
	,setCam: function(d) {
		...
		this._camera.setFOV(d.fov);
		this._camera.setPolar(d.amp,d.radX,d.radY,d.offsetY);
	}
	,__class__: camera_CamAngle
};
var camera_CamData = function(n,a,rx,ry,fovv,oY,gyak) {
	if(gyak == null) gyak = false;
	this.gyaku = false;
	this.offsetY = 0;
	this.fov = 60;
	this.amp = 0;
...
camera_CamData.getAngle = function() {
	camera_CamData.index++;
	return camera_CamData.cams[camera_CamData.index % camera_CamData.cams.length];
};
...
camera_ExCamera.prototype = $extend(THREE.PerspectiveCamera.prototype,{
	init: function(dom) {
		...
		this._isMobile = utils_OsChecker.isMobile();
		if(!this._isMobile) {
	...
	,onMouseDown: function(e) {
		...
		if(!this._isMobile) {
			...
			this._mouseX = this._downX = e.changedTouches[0].pageX;
			this._mouseY = this._downY = e.changedTouches[0].pageY;
		}
	...
	,update: function() {
		...
		if(this._camera != null) this._updatePosition(1);
	}
	,updateFOV: function() {
		this._camera.updateProjectionMatrix();
	}
...
common_Dat._onInit = function() {
	...
	common_Dat.gui.domElement.style.top = "0px";
...
data_ColorData.__name__ = true;
data_ColorData.getNext = function() {
	if(data_ColorData._index == -1) data_ColorData._index = Math.floor(data_ColorData._colors.length * Math.random());
	data_ColorData._index++;
	return data_ColorData._colors[data_ColorData._index % data_ColorData._colors.length];
};
data_ColorData.prototype = {
	__class__: data_ColorData
};
...
data_DataManager.prototype = {
	...
	,_onLoad3: function() {
		this.nyan = new loaders_MyBoxLoader();
		this.nyan.load("./obj/moji/moji.dae",$bind(this,this._onLoad4));
	}
	,_onLoad4: function() {
		this.xmas = new loaders_MyBoxLoader();
		this.xmas.load("./obj/moji/xmas.dae",$bind(this,this._onLoad5));
	}
	,_onLoad5: function() {
		this.nyangle = new loaders_MyBoxLoader();
		this.nyangle.load("./obj/moji/nyanglebell.dae",$bind(this,this._onLoad6));
	}
	,_onLoad6: function() {
		if(this._callback != null) this._callback();
	}
...
var html_Page = function() {
};
html_Page.__name__ = true;
html_Page.init = function() {
	js.JQuery("#aboutBtn").mousedown(html_Page.showAbout);
};
html_Page.showAbout = function(j) {
	js.JQuery("#about").css({ display : "flex"});
	js.JQuery("#closeBtn").mousedown(html_Page.hideAbout);
};
html_Page.hideAbout = function(j) {
	js.JQuery("#about").hide();
};
html_Page.showLoading = function(delay) {
	html_Page._isHideLoading = false;
	TweenMax.delayedCall(delay,html_Page._onShowLoading);
};
html_Page._onShowLoading = function() {
	if(!html_Page._isHideLoading) {
		js.JQuery("#loading").show();
		js.JQuery("#loading").css({ backgroundColor : "rgba(0,0,0,0)"});
	}
};
html_Page.hideLoading = function() {
	html_Page._isHideLoading = true;
	js.JQuery("#loading").hide();
};
html_Page.showStart = function(onTouchStart) {
	html_Page._startCallback = onTouchStart;
	js.JQuery("#btn").mousedown(html_Page._onShowStart);
};
html_Page._onShowStart = function() {
	if(html_Page._startCallback != null) {
		html_Page._startCallback();
		html_Page._startCallback = null;
	}
};
html_Page.hideStart = function() {
	if(html_Page._isStart) return;
	html_Page._isStart = true;
	js.JQuery("#btn").mousedown(null);
	js.JQuery("#start").animate({ opacity : 0},3000,html_Page._onHideStart);
};
html_Page._onHideStart = function() {
	js.JQuery("#start").hide();
};
html_Page.prototype = {
	__class__: html_Page
};
...
loaders_MyBoxLoader.prototype = $extend(loaders_MyDAELoader.prototype,{
	...
	,_onComplete: function(ret) {
		var dae = ret.scene;
		this.mat = new THREE.MeshLambertMaterial({ color : 16711680, ambient : 16777215, side : 2});
		this.mat.shading = 2;
...
var objects_CatBody = function() {
	...
	if(objects_CatBody._mate == null) objects_CatBody._mate = new shaders_MaeShaderMaterial("obj/cat/body.png",4);
...
objects_LongCat.prototype = $extend(THREE.Object3D.prototype,{
	init: function() {
		...
		this._hat = new objects_cat_CatHat();
		this._hat.rotation.x = -Math.PI / 2;
		this._hat.position.y = -0.1;
		this._hat.position.z = -0.35;
		this._hat.visible = false;
		this._cat.add(this._hat);
		this._body = new objects_CatBody();
		this._body.scale.set(100,0.001,100);
		this._body.init();
		this._body.position.y = -63.743;
...
objects_XmasGenerator.prototype = $extend(THREE.Object3D.prototype,{
	...
	,setPaperColor: function(col) {
		this._cats.setPaperColor(col);
	}
...
var objects_bg_TimelineMesh = function() {
	var geo = new THREE.Geometry();
	var ww = 2000;
	var dd = data_Params.distance;
	geo.vertices.push(new THREE.Vector3(-ww / 2,0,0));
	geo.vertices.push(new THREE.Vector3(ww / 2,0,0));
	geo.vertices.push(new THREE.Vector3(-ww / 2,0,-dd / 2));
	geo.vertices.push(new THREE.Vector3(ww / 2,0,-dd / 2));
	geo.vertices.push(new THREE.Vector3(-ww / 2,0,dd / 2));
	geo.vertices.push(new THREE.Vector3(ww / 2,0,dd / 2));
	geo.vertices.push(new THREE.Vector3(-ww / 2,0,-dd / 2));
	...
	geo.vertices.push(new THREE.Vector3(ww / 2,0,-dd / 2));
	geo.vertices.push(new THREE.Vector3(ww / 2,0,dd / 2));
	this._mat = new THREE.LineBasicMaterial({ color : 16776960});
	THREE.LineSegments.call(this,geo,this._mat);
	this.frustumCulled = false;
};
objects_bg_TimelineMesh.__name__ = true;
objects_bg_TimelineMesh.__super__ = THREE.LineSegments;
objects_bg_TimelineMesh.prototype = $extend(THREE.LineSegments.prototype,{
	update: function(rr) {
		var zz = data_Params.distance * (rr - 0.5);
	...
	,setColor: function(col) {
		this._mat.color.r = col.r;
		this._mat.color.g = col.g;
		this._mat.color.b = col.b;
	}
	,__class__: objects_bg_TimelineMesh
});
var objects_bg_XmasLogo = function(geo) {
	this._index = 0;
	this.colors = [new THREE.Color(1,0,0),new THREE.Color(1,1,0),new THREE.Color(0,1,0),new THREE.Color(0,1,1),new THREE.Color(0,0,1),new THREE.Color(1,0,1)];
	this._geo = geo;
	this._mat = new THREE.MeshLambertMaterial({ color : 16711680});
	THREE.Mesh.call(this,this._geo,this._mat);
};
objects_bg_XmasLogo.__name__ = true;
objects_bg_XmasLogo.__super__ = THREE.Mesh;
objects_bg_XmasLogo.prototype = $extend(THREE.Mesh.prototype,{
	setColor: function() {
		var tgt = this.colors[this._index % this.colors.length];
		this._index++;
		this._mat.color.copy(tgt);
	}
	,__class__: objects_bg_XmasLogo
});
var objects_cat_CatHat = function() {
	if(objects_cat_CatHat._geo == null) {
		objects_cat_CatHat._geo = new THREE.ConeGeometry(0.08,0.3);
		objects_cat_CatHat._mat = new THREE.MeshLambertMaterial({ color : 16711680});
	}
	THREE.Mesh.call(this,objects_cat_CatHat._geo,objects_cat_CatHat._mat);
};
objects_cat_CatHat.__name__ = true;
objects_cat_CatHat.__super__ = THREE.Mesh;
objects_cat_CatHat.prototype = $extend(THREE.Mesh.prototype,{
	__class__: objects_cat_CatHat
});
var objects_cat_GiftBox = function() {
	THREE.Object3D.call(this);
	if(objects_cat_GiftBox._mat == null) {
		objects_cat_GiftBox._mat = data_DataManager.getInstance().box.mat;
		objects_cat_GiftBox._geo1 = data_DataManager.getInstance().box.geos[0];
		objects_cat_GiftBox._geo2 = data_DataManager.getInstance().box.geos[1];
	}
	this._top = new THREE.Mesh(objects_cat_GiftBox._geo1,objects_cat_GiftBox._mat);
	this._bottom = new THREE.Mesh(objects_cat_GiftBox._geo2,objects_cat_GiftBox._mat);
	this._bottom.castShadow = true;
	this.add(this._top);
	this.add(this._bottom);
	this.reset();
};
objects_cat_GiftBox.__name__ = true;
objects_cat_GiftBox.__super__ = THREE.Object3D;
objects_cat_GiftBox.prototype = $extend(THREE.Object3D.prototype,{
	open: function() {
		TweenMax.to(this._top.position,0.5,{ x : 200 * (Math.random() - 0.5), y : this._top.position.y + 300 + 200 * Math.random()});
...
objects_cat_GiftBoxCat.prototype = $extend(THREE.Object3D.prototype,{
	init: function(d) {
		this.data = d;
		this._box = new objects_cat_GiftBox();
		this._box.position.y = 50;
		this.add(this._box);
		this._papers = new particles_papers_KamiParticle();
		this.add(this._papers);
		this._cat = new objects_LongCat();
		this._cat.init();
		this._cat.setSize(200 * Math.random());
		this._rotSpeed = (Math.random() - 0.5) * 0.5;
		this._cat.rotation.y = Math.random() * Math.PI * 2;
	...
	,update: function() {
		...
		this._cat.position.y += this._ySpeed;
		this._cat.rotation.y += this._rotSpeed;
	}
	,reset: function() {
		this._cat.visible = false;
		this._box.reset();
...
objects_cat_MidiCats.prototype = $extend(THREE.Object3D.prototype,{
	...
	,removeCats: function(list) {
		...
		while(_g11 < _g2) {
			var i1 = _g11++;
			var data1 = list[i1];
			var timeRatio = data1.time / 17.;
			var cat = new objects_cat_GiftBoxCat();
			cat.init(data1);
			cat.position.x = (this.getX(data1) - 2.5) * 300;
			cat.position.y = 0;
	...
	,setPaperColor: function(col) {
		...
		while(_g1 < _g) {
			var i = _g1++;
			this._cats[i].getPaper().setColor(col);
		}
	}
...
var particles_MojiGeo = function(geo,scale,num) {
	...
	var ids = new Float32Array(vertexPositions.length * num);
	...
	while(_g2 < num) {
		...
		while(_g11 < pointsLen) {
			...
			ids[j * pointsLen + i1] = Math.floor(j / (num * 0.1));
		}
	}
	...
	this.addAttribute("idd",new THREE.BufferAttribute(ids,1));
};
...
var particles_MojiParticles = function() {
	THREE.Object3D.call(this);
};
particles_MojiParticles.__name__ = true;
particles_MojiParticles.__super__ = THREE.Object3D;
particles_MojiParticles.prototype = $extend(THREE.Object3D.prototype,{
	init: function() {
		var geo = new particles_MojiGeo(data_DataManager.getInstance().nyan.geos[0],0.5,400);
	...
	,addNumRatio: function() {
		this._mat.addNum();
	}
	,start: function() {
		this._mat.start();
	}
	,__class__: particles_MojiParticles
});
var particles_PaperParticlesMat = function() {
	...
	THREE.ShaderMaterial.call(this,{ vertexShader : this.vv, fragmentShader : this.ff, uniforms : { map : { type : "t", value : null}, counter : { type : "f", value : 0}, rotRatio : { type : "f", value : 0.9999}, numRatio : { type : "f", value : 0.9999}, colRatio : { type : "f", value : 0.9999}, sclRatio : { type : "f", value : 0.9999}, sclRatio2 : { type : "f", value : 0.9999}, rotAxis : { type : "v3", value : new THREE.Vector3(1,0,0)}, col : { type : "v3", value : new THREE.Vector3(1,0,0)}, offsetColor : { type : "v3", value : new THREE.Vector3()}}});
	this.side = 2;
};
...
particles_PaperParticlesMat.prototype = $extend(THREE.ShaderMaterial.prototype,{
	...
	,_onNote: function(e) {
		var n = e.data;
		if(n.type == "BEAT") {
			if(n.name == "C2") this.uniforms.sclRatio2.value = 1.4;
		}
	}
	,setColor: function(col) {
		this.uniforms.col.value.x = col.r;
		this.uniforms.col.value.y = col.g;
		this.uniforms.col.value.z = col.b;
		this.uniforms.counter.value = 0;
	}
	,reset: function() {
		this._num = 0;
		this.uniforms.sclRatio.value = 0;
		if(Math.random() < 0.7) this._resetMode = true; else this._resetMode = false;
	}
	,flash: function() {
		if(this._twn != null) this._twn.kill();
		if(this._twn2 != null) this._twn2.kill();
		if(this._resetMode) {
			this.uniforms.rotRatio.value = 1.5;
			this._twn2 = TweenMax.to(this.uniforms.rotRatio,0.2,{ value : 0, ease : Power0.easeInOut});
			this.uniforms.rotAxis.value.x = Math.random() - 0.5;
			this.uniforms.rotAxis.value.y = Math.random() - 0.5;
			this.uniforms.rotAxis.value.z = Math.random() - 0.5;
		} else {
			this.uniforms.rotRatio.value = 1.0;
			this._twn2 = TweenMax.to(this.uniforms.rotAxis.value,0.1,{ x : 2 * (Math.random() - 0.5), y : 2 * (Math.random() - 0.5), z : 2 * (Math.random() - 0.5), ease : Power0.easeInOut});
		}
		this._flag = true;
		this._num += 0.1;
		this.uniforms.numRatio = 1;
		TweenMax.to(this.uniforms.sclRatio,0.1,{ value : this._num, ease : Power0.easeInOut, onComplete : $bind(this,this._onFlash)});
	}
	,_onFlash: function() {
		this._flag = false;
	}
	,update: function() {
		...
		this.uniforms.sclRatio2.value += (1.0 - this.uniforms.sclRatio2.value) / 4;
	}
	,addNum: function() {
	}
	,__class__: particles_PaperParticlesMat
});
var particles_StarParticle = function() {
	THREE.Object3D.call(this);
	var geometry = new THREE.Geometry();
	var size = 12000;
	var colors = [new THREE.Color(16711680),new THREE.Color(65280),new THREE.Color(16776960)];
	var _g = 0;
	while(_g < 3000) {
		var i = _g++;
	...
	var material = new THREE.PointsMaterial({ color : 16777215, size : 50, map : THREE.ImageUtils.loadTexture("./img/star.png"), vertexColors : 2, transparent : true});
	var points = new THREE.Points(geometry,material);
	this.add(points);
	points.position.z = data_Params.distance / 2;
	points.frustumCulled = false;
};
particles_StarParticle.__name__ = true;
particles_StarParticle.__super__ = THREE.Object3D;
particles_StarParticle.prototype = $extend(THREE.Object3D.prototype,{
	__class__: particles_StarParticle
});
var particles_papers_KamiGeo = function(num) {
	THREE.BufferGeometry.call(this);
	var s = 7;
...
particles_papers_KamiGeo.__name__ = true;
particles_papers_KamiGeo.__super__ = THREE.BufferGeometry;
particles_papers_KamiGeo.prototype = $extend(THREE.BufferGeometry.prototype,{
	__class__: particles_papers_KamiGeo
});
var particles_papers_KamiMat = function() {
	this._tgtSpeed = 0;
	this._speed = 0;
	...
	THREE.ShaderMaterial.call(this,{ vertexShader : this.vv, fragmentShader : this.ff, uniforms : { map : { type : "t", value : null}, counter : { type : "f", value : 0}, ratio : { type : "f", value : 0}, rotRatio : { type : "f", value : 0.9999}, numRatio : { type : "f", value : 0.9999}, colRatio : { type : "f", value : 0.9999}, sclRatio : { type : "f", value : 0.9999}, col : { type : "v3", value : new THREE.Vector3(1,0,0)}, offsetColor : { type : "v3", value : new THREE.Vector3()}}});
	this.side = 2;
};
particles_papers_KamiMat.__name__ = true;
particles_papers_KamiMat.__super__ = THREE.ShaderMaterial;
particles_papers_KamiMat.prototype = $extend(THREE.ShaderMaterial.prototype,{
	setColor: function(col) {
		this.uniforms.col.value.x = col.r;
		this.uniforms.col.value.y = col.g;
		this.uniforms.col.value.z = col.b;
	}
	,play: function(onComp) {
		this.uniforms.ratio.value = 0;
		TweenMax.to(this.uniforms.ratio,2.0,{ value : 0.5, ease : Power4.easeOut});
		this.uniforms.sclRatio.value = 1.0;
		TweenMax.to(this.uniforms.sclRatio,2.0,{ value : 0, ease : Cubic.easeIn, onComplete : onComp});
	}
	,update: function() {
	}
	,__class__: particles_papers_KamiMat
});
var particles_papers_KamiMesh = function() {
	THREE.Object3D.call(this);
};
particles_papers_KamiMesh.__name__ = true;
particles_papers_KamiMesh.__super__ = THREE.Object3D;
particles_papers_KamiMesh.prototype = $extend(THREE.Object3D.prototype,{
	init: function() {
		if(particles_papers_KamiMesh._geo == null) particles_papers_KamiMesh._geo = new particles_papers_KamiGeo(80);
		this._mat = new particles_papers_KamiMat();
		var m = new THREE.Mesh(particles_papers_KamiMesh._geo,this._mat);
		this.add(m);
	...
	,update: function() {
		if(this._mat != null) this._mat.update();
	}
	,play: function() {
		this.visible = true;
		this._mat.play($bind(this,this._onPlay));
	}
	,_onPlay: function() {
		this.visible = false;
	}
	,__class__: particles_papers_KamiMesh
});
var particles_papers_KamiParticle = function() {
	this._mat = new particles_papers_KamiParticleMat();
	THREE.Points.call(this,new particles_papers_KamiParticleGeo(30),this._mat);
	this.visible = false;
};
particles_papers_KamiParticle.__name__ = true;
particles_papers_KamiParticle.__super__ = THREE.Points;
particles_papers_KamiParticle.prototype = $extend(THREE.Points.prototype,{
	play: function() {
		Tracer.info("Play");
		this.visible = true;
		this._mat.play($bind(this,this._onPlay));
	...
	,__class__: particles_papers_KamiParticle
});
var particles_papers_KamiParticleGeo = function(num) {
	THREE.BufferGeometry.call(this);
	var s = 7;
	var vertexPositions_0 = [-1. * s,-1. * s,0.0 * s];
	var vertexPositions_1 = [s,-1. * s,0.0 * s];
	var vertexPositions_2 = [s,s,0.0 * s];
	var vertexPositions_3 = [-1. * s,s,0.0 * s];
	var normals = new Float32Array(12 * num);
	var vertices = new Float32Array(12 * num);
	var random = new Float32Array(12 * num);
	var color = new Float32Array(12 * num);
	var indices = new Uint32Array(6 * num);
	var _g = 0;
	while(_g < num) {
		var j = _g++;
...
particles_papers_KamiParticleGeo.__name__ = true;
particles_papers_KamiParticleGeo.__super__ = THREE.BufferGeometry;
particles_papers_KamiParticleGeo.prototype = $extend(THREE.BufferGeometry.prototype,{
	__class__: particles_papers_KamiParticleGeo
});
var particles_papers_KamiParticleMat = function() {
	this._tgtSpeed = 0;
	this._speed = 0;
	this.ff = "\r\n//\r\n// Description : Array and textureless GLSL 2D/3D/4D simplex \r\n//               noise functions.\r\n//      Author : Ian McEwan, Ashima Arts.\r\n//  Maintainer : ijm\r\n//     Lastmod : 20110822 (ijm)\r\n//     License : Copyright (C) 2011 Ashima Arts. All rights reserved.\r\n//               Distributed under the MIT License. See LICENSE file.\r\n//               https://github.com/ashima/webgl-noise\r\n// \r\n\r\nvec3 mod289(vec3 x) {\r\n\treturn x - floor(x * (1.0 / 289.0)) * 289.0;\r\n}\r\n\r\nvec4 mod289(vec4 x) {\r\n\treturn x - floor(x * (1.0 / 289.0)) * 289.0;\r\n}\r\n\r\nvec4 permute(vec4 x) {\r\n\treturn mod289(((x*34.0)+1.0)*x);\r\n}\r\n\r\nvec4 taylorInvSqrt(vec4 r){\r\n\treturn 1.79284291400159 - 0.85373472095314 * r;\r\n}\r\n\r\nfloat snoise(vec3 v) { \r\n\r\n\tconst vec2  C = vec2(1.0/6.0, 1.0/3.0) ;\r\n\tconst vec4  D = vec4(0.0, 0.5, 1.0, 2.0);\r\n\r\n\t// First corner\r\n\tvec3 i  = floor(v + dot(v, C.yyy) );\r\n\tvec3 x0 =   v - i + dot(i, C.xxx) ;\r\n\r\n\t// Other corners\r\n\tvec3 g = step(x0.yzx, x0.xyz);\r\n\tvec3 l = 1.0 - g;\r\n\tvec3 i1 = min( g.xyz, l.zxy );\r\n\tvec3 i2 = max( g.xyz, l.zxy );\r\n\r\n\t//   x0 = x0 - 0.0 + 0.0 * C.xxx;\r\n\t//   x1 = x0 - i1  + 1.0 * C.xxx;\r\n\t//   x2 = x0 - i2  + 2.0 * C.xxx;\r\n\t//   x3 = x0 - 1.0 + 3.0 * C.xxx;\r\n\tvec3 x1 = x0 - i1 + C.xxx;\r\n\tvec3 x2 = x0 - i2 + C.yyy; // 2.0*C.x = 1/3 = C.y\r\n\tvec3 x3 = x0 - D.yyy;      // -1.0+3.0*C.x = -0.5 = -D.y\r\n\r\n\t// Permutations\r\n\ti = mod289(i); \r\n\tvec4 p = permute( permute( permute( \r\n\t\t  i.z + vec4(0.0, i1.z, i2.z, 1.0 ))\r\n\t\t+ i.y + vec4(0.0, i1.y, i2.y, 1.0 )) \r\n\t\t+ i.x + vec4(0.0, i1.x, i2.x, 1.0 ));\r\n\r\n\t// Gradients: 7x7 points over a square, mapped onto an octahedron.\r\n\t// The ring size 17*17 = 289 is close to a multiple of 49 (49*6 = 294)\r\n\tfloat n_ = 0.142857142857; // 1.0/7.0\r\n\tvec3  ns = n_ * D.wyz - D.xzx;\r\n\r\n\tvec4 j = p - 49.0 * floor(p * ns.z * ns.z);  //  mod(p,7*7)\r\n\r\n\tvec4 x_ = floor(j * ns.z);\r\n\tvec4 y_ = floor(j - 7.0 * x_ );    // mod(j,N)\r\n\r\n\tvec4 x = x_ *ns.x + ns.yyyy;\r\n\tvec4 y = y_ *ns.x + ns.yyyy;\r\n\tvec4 h = 1.0 - abs(x) - abs(y);\r\n\r\n\tvec4 b0 = vec4( x.xy, y.xy );\r\n\tvec4 b1 = vec4( x.zw, y.zw );\r\n\r\n\t//vec4 s0 = vec4(lessThan(b0,0.0))*2.0 - 1.0;\r\n\t//vec4 s1 = vec4(lessThan(b1,0.0))*2.0 - 1.0;\r\n\tvec4 s0 = floor(b0)*2.0 + 1.0;\r\n\tvec4 s1 = floor(b1)*2.0 + 1.0;\r\n\tvec4 sh = -step(h, vec4(0.0));\r\n\r\n\tvec4 a0 = b0.xzyw + s0.xzyw*sh.xxyy ;\r\n\tvec4 a1 = b1.xzyw + s1.xzyw*sh.zzww ;\r\n\r\n\tvec3 p0 = vec3(a0.xy,h.x);\r\n\tvec3 p1 = vec3(a0.zw,h.y);\r\n\tvec3 p2 = vec3(a1.xy,h.z);\r\n\tvec3 p3 = vec3(a1.zw,h.w);\r\n\r\n\t//Normalise gradients\r\n\tvec4 norm = taylorInvSqrt(vec4(dot(p0,p0), dot(p1,p1), dot(p2, p2), dot(p3,p3)));\r\n\tp0 *= norm.x;\r\n\tp1 *= norm.y;\r\n\tp2 *= norm.z;\r\n\tp3 *= norm.w;\r\n\r\n\t// Mix final noise value\r\n\tvec4 m = max(0.6 - vec4(dot(x0,x0), dot(x1,x1), dot(x2,x2), dot(x3,x3)), 0.0);\r\n\tm = m * m;\r\n\treturn 42.0 * dot( m*m, vec4( dot(p0,x0), dot(p1,x1), dot(p2,x2), dot(p3,x3) ) );\r\n\r\n}\r\n\r\nvec3 snoiseVec3( vec3 x ){\r\n\r\n\tfloat s  = snoise(vec3( x ));\r\n\tfloat s1 = snoise(vec3( x.y - 19.1 , x.z + 33.4 , x.x + 47.2 ));\r\n\tfloat s2 = snoise(vec3( x.z + 74.2 , x.x - 124.5 , x.y + 99.4 ));\r\n\tvec3 c = vec3( s , s1 , s2 );\r\n\treturn c;\r\n\r\n}\r\n\r\nvec3 snoiseVec3Abs( vec3 x ){\r\n\r\n\tfloat s  = snoise(vec3( x ));\r\n\tfloat s1 = snoise(vec3( x.y - 19.1 , x.z + 33.4 , x.x + 47.2 ));\r\n\tfloat s2 = snoise(vec3( x.z + 74.2 , x.x - 124.5 , x.y + 99.4 ));\r\n\tvec3 c = vec3( abs(s) , abs(s1) , abs(s2) );\r\n\treturn c;\r\n\r\n}\r\n\r\n\r\nvec3 curlNoise( vec3 p ){\r\n \r\n\tconst float e = .1;\r\n\tvec3 dx = vec3( e   , 0.0 , 0.0 );\r\n\tvec3 dy = vec3( 0.0 , e   , 0.0 );\r\n\tvec3 dz = vec3( 0.0 , 0.0 , e   );\r\n\r\n\tvec3 p_x0 = snoiseVec3( p - dx );\r\n\tvec3 p_x1 = snoiseVec3( p + dx );\r\n\tvec3 p_y0 = snoiseVec3( p - dy );\r\n\tvec3 p_y1 = snoiseVec3( p + dy );\r\n\tvec3 p_z0 = snoiseVec3( p - dz );\r\n\tvec3 p_z1 = snoiseVec3( p + dz );\r\n\r\n\tfloat x = p_y1.z - p_y0.z - p_z1.y + p_z0.y;\r\n\tfloat y = p_z1.x - p_z0.x - p_x1.z + p_x0.z;\r\n\tfloat z = p_x1.y - p_x0.y - p_y1.x + p_y0.x;\r\n\r\n\tconst float divisor = 1.0 / ( 2.0 * e );\r\n\treturn normalize( vec3( x , y , z ) * divisor );\r\n\r\n}\r\n\r\nvec3 curlNoise2( vec3 p ) {\r\n\r\n\tconst float e = .1;\r\n\r\n\tvec3 xNoisePotentialDerivatives = snoiseVec3( p );\r\n\tvec3 yNoisePotentialDerivatives = snoiseVec3( p + e * vec3( 3., -3.,  1. ) );\r\n\tvec3 zNoisePotentialDerivatives = snoiseVec3( p + e * vec3( 2.,  4., -3. ) );\r\n\r\n\tvec3 noiseVelocity = vec3(\r\n\t\tzNoisePotentialDerivatives.y - yNoisePotentialDerivatives.z,\r\n\t\txNoisePotentialDerivatives.z - zNoisePotentialDerivatives.x,\r\n\t\tyNoisePotentialDerivatives.x - xNoisePotentialDerivatives.y\r\n\t);\r\n\r\n\treturn normalize( noiseVelocity );\r\n\r\n}\r\n\r\nvec4 snoiseD(vec3 v) { //returns vec4(value, dx, dy, dz)\r\n  const vec2  C = vec2(1.0/6.0, 1.0/3.0) ;\r\n  const vec4  D = vec4(0.0, 0.5, 1.0, 2.0);\r\n \r\n  vec3 i  = floor(v + dot(v, C.yyy) );\r\n  vec3 x0 =   v - i + dot(i, C.xxx) ;\r\n \r\n  vec3 g = step(x0.yzx, x0.xyz);\r\n  vec3 l = 1.0 - g;\r\n  vec3 i1 = min( g.xyz, l.zxy );\r\n  vec3 i2 = max( g.xyz, l.zxy );\r\n \r\n  vec3 x1 = x0 - i1 + C.xxx;\r\n  vec3 x2 = x0 - i2 + C.yyy;\r\n  vec3 x3 = x0 - D.yyy;\r\n \r\n  i = mod289(i);\r\n  vec4 p = permute( permute( permute(\r\n             i.z + vec4(0.0, i1.z, i2.z, 1.0 ))\r\n           + i.y + vec4(0.0, i1.y, i2.y, 1.0 ))\r\n           + i.x + vec4(0.0, i1.x, i2.x, 1.0 ));\r\n \r\n  float n_ = 0.142857142857; // 1.0/7.0\r\n  vec3  ns = n_ * D.wyz - D.xzx;\r\n \r\n  vec4 j = p - 49.0 * floor(p * ns.z * ns.z);\r\n \r\n  vec4 x_ = floor(j * ns.z);\r\n  vec4 y_ = floor(j - 7.0 * x_ );\r\n \r\n  vec4 x = x_ *ns.x + ns.yyyy;\r\n  vec4 y = y_ *ns.x + ns.yyyy;\r\n  vec4 h = 1.0 - abs(x) - abs(y);\r\n \r\n  vec4 b0 = vec4( x.xy, y.xy );\r\n  vec4 b1 = vec4( x.zw, y.zw );\r\n \r\n  vec4 s0 = floor(b0)*2.0 + 1.0;\r\n  vec4 s1 = floor(b1)*2.0 + 1.0;\r\n  vec4 sh = -step(h, vec4(0.0));\r\n \r\n  vec4 a0 = b0.xzyw + s0.xzyw*sh.xxyy ;\r\n  vec4 a1 = b1.xzyw + s1.xzyw*sh.zzww ;\r\n \r\n  vec3 p0 = vec3(a0.xy,h.x);\r\n  vec3 p1 = vec3(a0.zw,h.y);\r\n  vec3 p2 = vec3(a1.xy,h.z);\r\n  vec3 p3 = vec3(a1.zw,h.w);\r\n \r\n  vec4 norm = taylorInvSqrt(vec4(dot(p0,p0), dot(p1,p1), dot(p2, p2), dot(p3,p3)));\r\n  p0 *= norm.x;\r\n  p1 *= norm.y;\r\n  p2 *= norm.z;\r\n  p3 *= norm.w;\r\n \r\n  vec4 values = vec4( dot(p0,x0), dot(p1,x1), dot(p2,x2), dot(p3,x3) ); //value of contributions from each corner (extrapolate the gradient)\r\n \r\n  vec4 m = max(0.5 - vec4(dot(x0,x0), dot(x1,x1), dot(x2,x2), dot(x3,x3)), 0.0); //kernel function from each corner\r\n \r\n  vec4 m2 = m * m;\r\n  vec4 m3 = m * m * m;\r\n \r\n  vec4 temp = -6.0 * m2 * values;\r\n  float dx = temp[0] * x0.x + temp[1] * x1.x + temp[2] * x2.x + temp[3] * x3.x + m3[0] * p0.x + m3[1] * p1.x + m3[2] * p2.x + m3[3] * p3.x;\r\n  float dy = temp[0] * x0.y + temp[1] * x1.y + temp[2] * x2.y + temp[3] * x3.y + m3[0] * p0.y + m3[1] * p1.y + m3[2] * p2.y + m3[3] * p3.y;\r\n  float dz = temp[0] * x0.z + temp[1] * x1.z + temp[2] * x2.z + temp[3] * x3.z + m3[0] * p0.z + m3[1] * p1.z + m3[2] * p2.z + m3[3] * p3.z;\r\n \r\n  return vec4(dot(m3, values), dx, dy, dz) * 42.0;\r\n}\r\n\r\n\r\nvec3 curlNoise3 (vec3 p) {\r\n\r\n    vec3 xNoisePotentialDerivatives = snoiseD( p ).yzw; //yzw are the xyz derivatives\r\n    vec3 yNoisePotentialDerivatives = snoiseD(vec3( p.y - 19.1 , p.z + 33.4 , p.x + 47.2 )).zwy;\r\n    vec3 zNoisePotentialDerivatives = snoiseD(vec3( p.z + 74.2 , p.x - 124.5 , p.y + 99.4 )).wyz;\r\n    vec3 noiseVelocity = vec3(\r\n        zNoisePotentialDerivatives.y - yNoisePotentialDerivatives.z,\r\n        xNoisePotentialDerivatives.z - zNoisePotentialDerivatives.x,\r\n        yNoisePotentialDerivatives.x - xNoisePotentialDerivatives.y\r\n    );\r\n\t\r\n\tconst float e = .1;\r\n\tconst float divisor = 1.0 / ( 2.0 * e );\r\n\treturn normalize( noiseVelocity * divisor );\r\n\r\n}\r\n\t\r\n\t" + "\r\n\t\t//uniform 変数としてテクスチャのデータを受け取る\r\n\t\t//uniform sampler2D texture;\r\n\t\tuniform float counter;\r\n\t\tuniform vec3 col;\r\n\t\tuniform vec3 offsetColor;\r\n\t\tuniform float colRatio;\r\n\t\tuniform float numRatio;\r\n\t\tuniform sampler2D texture;\r\n\t\t// vertexShaderで処理されて渡されるテクスチャ座標\r\n\t\tvarying vec2 vUv;                                             \r\n\t\tvarying vec3 vNormal; \r\n\t\tvarying vec3 vColor;\r\n\t\tvarying float vId;\r\n\t\tvarying float vRatio;\r\n\t\t\r\n\t\tvoid main()\r\n\t\t{\r\n\t\t\t\r\n\t\t\t//if ( vId > numRatio ) discard;\r\n\t\t\tif ( colRatio <= 0.001) discard;\r\n\t\t\tif ( vRatio <= 0.0) discard;\r\n\t\t\tif ( vRatio >= 1.0) discard;\r\n\t\t\t\r\n\t\t\tvec4 color0 = texture2D( texture, gl_PointCoord  );\r\n\t\t\t\r\n\t\t\tgl_FragColor = vec4(color0.xyz * col, color0.a);\r\n\t\t\t\r\n\t\t}\r\n\t";
	this.vv = "\r\n//\r\n// Description : Array and textureless GLSL 2D/3D/4D simplex \r\n//               noise functions.\r\n//      Author : Ian McEwan, Ashima Arts.\r\n//  Maintainer : ijm\r\n//     Lastmod : 20110822 (ijm)\r\n//     License : Copyright (C) 2011 Ashima Arts. All rights reserved.\r\n//               Distributed under the MIT License. See LICENSE file.\r\n//               https://github.com/ashima/webgl-noise\r\n// \r\n\r\nvec3 mod289(vec3 x) {\r\n\treturn x - floor(x * (1.0 / 289.0)) * 289.0;\r\n}\r\n\r\nvec4 mod289(vec4 x) {\r\n\treturn x - floor(x * (1.0 / 289.0)) * 289.0;\r\n}\r\n\r\nvec4 permute(vec4 x) {\r\n\treturn mod289(((x*34.0)+1.0)*x);\r\n}\r\n\r\nvec4 taylorInvSqrt(vec4 r){\r\n\treturn 1.79284291400159 - 0.85373472095314 * r;\r\n}\r\n\r\nfloat snoise(vec3 v) { \r\n\r\n\tconst vec2  C = vec2(1.0/6.0, 1.0/3.0) ;\r\n\tconst vec4  D = vec4(0.0, 0.5, 1.0, 2.0);\r\n\r\n\t// First corner\r\n\tvec3 i  = floor(v + dot(v, C.yyy) );\r\n\tvec3 x0 =   v - i + dot(i, C.xxx) ;\r\n\r\n\t// Other corners\r\n\tvec3 g = step(x0.yzx, x0.xyz);\r\n\tvec3 l = 1.0 - g;\r\n\tvec3 i1 = min( g.xyz, l.zxy );\r\n\tvec3 i2 = max( g.xyz, l.zxy );\r\n\r\n\t//   x0 = x0 - 0.0 + 0.0 * C.xxx;\r\n\t//   x1 = x0 - i1  + 1.0 * C.xxx;\r\n\t//   x2 = x0 - i2  + 2.0 * C.xxx;\r\n\t//   x3 = x0 - 1.0 + 3.0 * C.xxx;\r\n\tvec3 x1 = x0 - i1 + C.xxx;\r\n\tvec3 x2 = x0 - i2 + C.yyy; // 2.0*C.x = 1/3 = C.y\r\n\tvec3 x3 = x0 - D.yyy;      // -1.0+3.0*C.x = -0.5 = -D.y\r\n\r\n\t// Permutations\r\n\ti = mod289(i); \r\n\tvec4 p = permute( permute( permute( \r\n\t\t  i.z + vec4(0.0, i1.z, i2.z, 1.0 ))\r\n\t\t+ i.y + vec4(0.0, i1.y, i2.y, 1.0 )) \r\n\t\t+ i.x + vec4(0.0, i1.x, i2.x, 1.0 ));\r\n\r\n\t// Gradients: 7x7 points over a square, mapped onto an octahedron.\r\n\t// The ring size 17*17 = 289 is close to a multiple of 49 (49*6 = 294)\r\n\tfloat n_ = 0.142857142857; // 1.0/7.0\r\n\tvec3  ns = n_ * D.wyz - D.xzx;\r\n\r\n\tvec4 j = p - 49.0 * floor(p * ns.z * ns.z);  //  mod(p,7*7)\r\n\r\n\tvec4 x_ = floor(j * ns.z);\r\n\tvec4 y_ = floor(j - 7.0 * x_ );    // mod(j,N)\r\n\r\n\tvec4 x = x_ *ns.x + ns.yyyy;\r\n\tvec4 y = y_ *ns.x + ns.yyyy;\r\n\tvec4 h = 1.0 - abs(x) - abs(y);\r\n\r\n\tvec4 b0 = vec4( x.xy, y.xy );\r\n\tvec4 b1 = vec4( x.zw, y.zw );\r\n\r\n\t//vec4 s0 = vec4(lessThan(b0,0.0))*2.0 - 1.0;\r\n\t//vec4 s1 = vec4(lessThan(b1,0.0))*2.0 - 1.0;\r\n\tvec4 s0 = floor(b0)*2.0 + 1.0;\r\n\tvec4 s1 = floor(b1)*2.0 + 1.0;\r\n\tvec4 sh = -step(h, vec4(0.0));\r\n\r\n\tvec4 a0 = b0.xzyw + s0.xzyw*sh.xxyy ;\r\n\tvec4 a1 = b1.xzyw + s1.xzyw*sh.zzww ;\r\n\r\n\tvec3 p0 = vec3(a0.xy,h.x);\r\n\tvec3 p1 = vec3(a0.zw,h.y);\r\n\tvec3 p2 = vec3(a1.xy,h.z);\r\n\tvec3 p3 = vec3(a1.zw,h.w);\r\n\r\n\t//Normalise gradients\r\n\tvec4 norm = taylorInvSqrt(vec4(dot(p0,p0), dot(p1,p1), dot(p2, p2), dot(p3,p3)));\r\n\tp0 *= norm.x;\r\n\tp1 *= norm.y;\r\n\tp2 *= norm.z;\r\n\tp3 *= norm.w;\r\n\r\n\t// Mix final noise value\r\n\tvec4 m = max(0.6 - vec4(dot(x0,x0), dot(x1,x1), dot(x2,x2), dot(x3,x3)), 0.0);\r\n\tm = m * m;\r\n\treturn 42.0 * dot( m*m, vec4( dot(p0,x0), dot(p1,x1), dot(p2,x2), dot(p3,x3) ) );\r\n\r\n}\r\n\r\nvec3 snoiseVec3( vec3 x ){\r\n\r\n\tfloat s  = snoise(vec3( x ));\r\n\tfloat s1 = snoise(vec3( x.y - 19.1 , x.z + 33.4 , x.x + 47.2 ));\r\n\tfloat s2 = snoise(vec3( x.z + 74.2 , x.x - 124.5 , x.y + 99.4 ));\r\n\tvec3 c = vec3( s , s1 , s2 );\r\n\treturn c;\r\n\r\n}\r\n\r\nvec3 snoiseVec3Abs( vec3 x ){\r\n\r\n\tfloat s  = snoise(vec3( x ));\r\n\tfloat s1 = snoise(vec3( x.y - 19.1 , x.z + 33.4 , x.x + 47.2 ));\r\n\tfloat s2 = snoise(vec3( x.z + 74.2 , x.x - 124.5 , x.y + 99.4 ));\r\n\tvec3 c = vec3( abs(s) , abs(s1) , abs(s2) );\r\n\treturn c;\r\n\r\n}\r\n\r\n\r\nvec3 curlNoise( vec3 p ){\r\n \r\n\tconst float e = .1;\r\n\tvec3 dx = vec3( e   , 0.0 , 0.0 );\r\n\tvec3 dy = vec3( 0.0 , e   , 0.0 );\r\n\tvec3 dz = vec3( 0.0 , 0.0 , e   );\r\n\r\n\tvec3 p_x0 = snoiseVec3( p - dx );\r\n\tvec3 p_x1 = snoiseVec3( p + dx );\r\n\tvec3 p_y0 = snoiseVec3( p - dy );\r\n\tvec3 p_y1 = snoiseVec3( p + dy );\r\n\tvec3 p_z0 = snoiseVec3( p - dz );\r\n\tvec3 p_z1 = snoiseVec3( p + dz );\r\n\r\n\tfloat x = p_y1.z - p_y0.z - p_z1.y + p_z0.y;\r\n\tfloat y = p_z1.x - p_z0.x - p_x1.z + p_x0.z;\r\n\tfloat z = p_x1.y - p_x0.y - p_y1.x + p_y0.x;\r\n\r\n\tconst float divisor = 1.0 / ( 2.0 * e );\r\n\treturn normalize( vec3( x , y , z ) * divisor );\r\n\r\n}\r\n\r\nvec3 curlNoise2( vec3 p ) {\r\n\r\n\tconst float e = .1;\r\n\r\n\tvec3 xNoisePotentialDerivatives = snoiseVec3( p );\r\n\tvec3 yNoisePotentialDerivatives = snoiseVec3( p + e * vec3( 3., -3.,  1. ) );\r\n\tvec3 zNoisePotentialDerivatives = snoiseVec3( p + e * vec3( 2.,  4., -3. ) );\r\n\r\n\tvec3 noiseVelocity = vec3(\r\n\t\tzNoisePotentialDerivatives.y - yNoisePotentialDerivatives.z,\r\n\t\txNoisePotentialDerivatives.z - zNoisePotentialDerivatives.x,\r\n\t\tyNoisePotentialDerivatives.x - xNoisePotentialDerivatives.y\r\n\t);\r\n\r\n\treturn normalize( noiseVelocity );\r\n\r\n}\r\n\r\nvec4 snoiseD(vec3 v) { //returns vec4(value, dx, dy, dz)\r\n  const vec2  C = vec2(1.0/6.0, 1.0/3.0) ;\r\n  const vec4  D = vec4(0.0, 0.5, 1.0, 2.0);\r\n \r\n  vec3 i  = floor(v + dot(v, C.yyy) );\r\n  vec3 x0 =   v - i + dot(i, C.xxx) ;\r\n \r\n  vec3 g = step(x0.yzx, x0.xyz);\r\n  vec3 l = 1.0 - g;\r\n  vec3 i1 = min( g.xyz, l.zxy );\r\n  vec3 i2 = max( g.xyz, l.zxy );\r\n \r\n  vec3 x1 = x0 - i1 + C.xxx;\r\n  vec3 x2 = x0 - i2 + C.yyy;\r\n  vec3 x3 = x0 - D.yyy;\r\n \r\n  i = mod289(i);\r\n  vec4 p = permute( permute( permute(\r\n             i.z + vec4(0.0, i1.z, i2.z, 1.0 ))\r\n           + i.y + vec4(0.0, i1.y, i2.y, 1.0 ))\r\n           + i.x + vec4(0.0, i1.x, i2.x, 1.0 ));\r\n \r\n  float n_ = 0.142857142857; // 1.0/7.0\r\n  vec3  ns = n_ * D.wyz - D.xzx;\r\n \r\n  vec4 j = p - 49.0 * floor(p * ns.z * ns.z);\r\n \r\n  vec4 x_ = floor(j * ns.z);\r\n  vec4 y_ = floor(j - 7.0 * x_ );\r\n \r\n  vec4 x = x_ *ns.x + ns.yyyy;\r\n  vec4 y = y_ *ns.x + ns.yyyy;\r\n  vec4 h = 1.0 - abs(x) - abs(y);\r\n \r\n  vec4 b0 = vec4( x.xy, y.xy );\r\n  vec4 b1 = vec4( x.zw, y.zw );\r\n \r\n  vec4 s0 = floor(b0)*2.0 + 1.0;\r\n  vec4 s1 = floor(b1)*2.0 + 1.0;\r\n  vec4 sh = -step(h, vec4(0.0));\r\n \r\n  vec4 a0 = b0.xzyw + s0.xzyw*sh.xxyy ;\r\n  vec4 a1 = b1.xzyw + s1.xzyw*sh.zzww ;\r\n \r\n  vec3 p0 = vec3(a0.xy,h.x);\r\n  vec3 p1 = vec3(a0.zw,h.y);\r\n  vec3 p2 = vec3(a1.xy,h.z);\r\n  vec3 p3 = vec3(a1.zw,h.w);\r\n \r\n  vec4 norm = taylorInvSqrt(vec4(dot(p0,p0), dot(p1,p1), dot(p2, p2), dot(p3,p3)));\r\n  p0 *= norm.x;\r\n  p1 *= norm.y;\r\n  p2 *= norm.z;\r\n  p3 *= norm.w;\r\n \r\n  vec4 values = vec4( dot(p0,x0), dot(p1,x1), dot(p2,x2), dot(p3,x3) ); //value of contributions from each corner (extrapolate the gradient)\r\n \r\n  vec4 m = max(0.5 - vec4(dot(x0,x0), dot(x1,x1), dot(x2,x2), dot(x3,x3)), 0.0); //kernel function from each corner\r\n \r\n  vec4 m2 = m * m;\r\n  vec4 m3 = m * m * m;\r\n \r\n  vec4 temp = -6.0 * m2 * values;\r\n  float dx = temp[0] * x0.x + temp[1] * x1.x + temp[2] * x2.x + temp[3] * x3.x + m3[0] * p0.x + m3[1] * p1.x + m3[2] * p2.x + m3[3] * p3.x;\r\n  float dy = temp[0] * x0.y + temp[1] * x1.y + temp[2] * x2.y + temp[3] * x3.y + m3[0] * p0.y + m3[1] * p1.y + m3[2] * p2.y + m3[3] * p3.y;\r\n  float dz = temp[0] * x0.z + temp[1] * x1.z + temp[2] * x2.z + temp[3] * x3.z + m3[0] * p0.z + m3[1] * p1.z + m3[2] * p2.z + m3[3] * p3.z;\r\n \r\n  return vec4(dot(m3, values), dx, dy, dz) * 42.0;\r\n}\r\n\r\n\r\nvec3 curlNoise3 (vec3 p) {\r\n\r\n    vec3 xNoisePotentialDerivatives = snoiseD( p ).yzw; //yzw are the xyz derivatives\r\n    vec3 yNoisePotentialDerivatives = snoiseD(vec3( p.y - 19.1 , p.z + 33.4 , p.x + 47.2 )).zwy;\r\n    vec3 zNoisePotentialDerivatives = snoiseD(vec3( p.z + 74.2 , p.x - 124.5 , p.y + 99.4 )).wyz;\r\n    vec3 noiseVelocity = vec3(\r\n        zNoisePotentialDerivatives.y - yNoisePotentialDerivatives.z,\r\n        xNoisePotentialDerivatives.z - zNoisePotentialDerivatives.x,\r\n        yNoisePotentialDerivatives.x - xNoisePotentialDerivatives.y\r\n    );\r\n\t\r\n\tconst float e = .1;\r\n\tconst float divisor = 1.0 / ( 2.0 * e );\r\n\treturn normalize( noiseVelocity * divisor );\r\n\r\n}\r\n\t\r\n\t" + "\r\n\t\tvarying vec2 vUv;// fragmentShaderに渡すためのvarying変数\r\n\t\tvarying vec3 vNormal;\r\n\t\tvarying vec3 vColor;\r\n\t\tattribute vec3 color; // 頂点カラー\r\n\t\tattribute vec3 random;\r\n\t\t\r\n\t\tattribute float idd;\t\t\r\n\t\tvarying float vId;\r\n\t\tvarying float vRatio;\r\n\t\t\r\n\t\tuniform float counter;\r\n\t\tuniform float ratio;\r\n\t\tuniform float rotRatio;\r\n\t\tuniform float sclRatio;\r\n\t\tuniform vec3 col;\r\n\t\t\r\n\t\tvec3 rotateVec3(vec3 p, float angle, vec3 axis){\r\n\t\t  vec3 a = normalize(axis);\r\n\t\t  float s = sin(angle);\r\n\t\t  float c = cos(angle);\r\n\t\t  float r = 1.0 - c;\r\n\t\t  mat3 m = mat3(\r\n\t\t\ta.x * a.x * r + c,\r\n\t\t\ta.y * a.x * r + a.z * s,\r\n\t\t\ta.z * a.x * r - a.y * s,\r\n\t\t\ta.x * a.y * r - a.z * s,\r\n\t\t\ta.y * a.y * r + c,\r\n\t\t\ta.z * a.y * r + a.x * s,\r\n\t\t\ta.x * a.z * r + a.y * s,\r\n\t\t\ta.y * a.z * r - a.x * s,\r\n\t\t\ta.z * a.z * r + c\r\n\t\t  );\r\n\t\t  return m * p;\r\n\t\t}\r\n\t\t\r\n\t\t\r\n\t\tvoid main()\r\n\t\t{\r\n\t\t\t\r\n\t\t\tfloat size = 500.0;\r\n\t\t\tfloat height = 500.0;\r\n\t\t\t//pos.z += radius;\r\n\t\t\t//vec3 pos = rotateVec3(position, counter, vec3(0.0, 1.0, 0.0)) + random * 300.0;\t\t\r\n\t\t\t//vec3 pos = rotateVec3( position, counter+random.x*100.0, normalize(random) ) + random * size;\t\t\r\n\t\t\tfloat rot = ratio * random.z * 100.0 + random.x * 100.0;\r\n\t\t\tvec3 pos = rotateVec3( position * sclRatio, rot * rotRatio, normalize(random) );\t\r\n\t\t\tvec3 norm = rotateVec3(normal, rot * rotRatio, normalize(random) );\r\n\t\t\t\r\n\t\t\t//1000下がる\r\n\t\t\t//float d = mod( abs(random.y) * fall + counter * (10.0+10.0*abs(random.x)), fall );\r\n\t\t\t\r\n\t\t\tfloat rr = ratio * ( 1.0 + abs( random.x + random.y ) / 2.0 ) - abs(random.z);\r\n\t\t\trr = clamp(rr, 0.0, 1.0);\r\n\t\t\t\r\n\t\t\tpos.x = pos.x + random.x * size * rr;\r\n\t\t\t//pos.y = pos.y + abs( random.y ) * 600.0 * ratio;// - d;\r\n\t\t\tpos.y = pos.y - 4.0 * rr * ( rr - 1.0 ) * (height*0.5 + height*0.5 * abs(random.y));\r\n\t\t\tpos.z = pos.z + random.z * size * rr;\r\n\t\t\t\r\n\t\t\tvColor = col;\r\n\t\t\tvId = idd;\r\n\t\t\tvNormal = normalMatrix * norm;\r\n\t\t\tvRatio = rr;\r\n\t\t\t\r\n\t\t\t// 処理する頂点ごとのuv(テクスチャ)座標をそのままfragmentShaderに横流しする\r\n\t\t\tvUv = uv;\r\n\t\t\t// 変換：ローカル座標 → 配置 → カメラ座標\r\n\t\t\tvec4 mvPosition = modelViewMatrix * vec4(pos, 1.0);    \r\n\t\t\t// 変換：カメラ座標 → 画面座標\r\n\t\t\tgl_Position = projectionMatrix * mvPosition;\r\n\t\t\tgl_PointSize = sclRatio * (7000.0 + 2000.0 * random.x ) / gl_Position.w;\r\n\t\t}\r\n\t";
	THREE.ShaderMaterial.call(this,{ vertexShader : this.vv, fragmentShader : this.ff, uniforms : { map : { type : "t", value : null}, counter : { type : "f", value : 0}, ratio : { type : "f", value : 0}, rotRatio : { type : "f", value : 0.9999}, numRatio : { type : "f", value : 0.9999}, colRatio : { type : "f", value : 0.9999}, sclRatio : { type : "f", value : 0.9999}, col : { type : "v3", value : new THREE.Vector3(1,0,0)}, texture : { type : "t", value : THREE.ImageUtils.loadTexture("./img/star.png")}, offsetColor : { type : "v3", value : new THREE.Vector3()}}});
	this.side = 2;
	this.transparent = true;
};
particles_papers_KamiParticleMat.__name__ = true;
particles_papers_KamiParticleMat.__super__ = THREE.ShaderMaterial;
particles_papers_KamiParticleMat.prototype = $extend(THREE.ShaderMaterial.prototype,{
	setColor: function(col) {
		this.uniforms.col.value.x = col.r;
		this.uniforms.col.value.y = col.g;
		this.uniforms.col.value.z = col.b;
	...
	,__class__: particles_papers_KamiParticleMat
});
var shaders_MaeShaderMaterial = function(uri,repeat) {
	...
	THREE.ShaderMaterial.call(this,{ vertexShader : this.vv, fragmentShader : this.ff, uniforms : { map : { type : "t", value : this._texture1}, rp : { type : "t", value : repeat}}});
	this.side = 2;
};
...
var sound_MyAudio = function() {
	...
	THREE.EventDispatcher.call(this);
	if(sound_MyAudio.a != null) return;
...
sound_MyAudio.prototype = $extend(THREE.EventDispatcher.prototype,{
	...
	,_init: function() {
		...
		this.audio.src = "sound/nyangle.mp3";
		window.document.body.appendChild(this.audio);
		this.audio.autoplay = false;
		this.audio.volume = 0.0;
		html_Page.showStart($bind(this,this._onTouchStart));
	}
	,pause: function() {
		this.audio.pause();
	}
	,_onTouchStart: function() {
		if(utils_OsChecker.isMobile()) html_Page.showLoading(0.1); else html_Page.showLoading(0.5);
		this.audio.addEventListener("ended",$bind(this,this._onEnd));
	...
	,_onLoad: function(e) {
		...
		common_Dat.gui.add(this.audio,"duration");
		html_Page.hideStart();
		html_Page.hideLoading();
		this._callback();
	}
	,play: function() {
		this.isStart = true;
		this.volume = 0;
		TweenMax.to(this,2,{ volume : 0.5, ease : Power0.easeInOut, onUpdate : $bind(this,this._onUpdate)});
		this.audio.play();
		this.update();
	}
	,_onUpdate: function() {
	...
	,_calcMidi: function() {
		...
		if(currentTime < this._oldTime) {
			Tracer.info("終了 " + currentTime + "/" + this._oldTime);
			this.dispatchEvent({ type : "finish"});
		}
		if(Math.abs(currentTime * 2 % 1 - this._oldTime * 2 % 1) > 0.9) this.dispatchEvent({ type : "500ms"});
		this._oldTime = currentTime;
	}
	,getTime: function() {
		return this.audio.currentTime % (this.audio.duration / 3);
	}
	,getTimeRatio: function() {
		return this.getTime() / (this.audio.duration / 3);
	}
	,getDuration: function() {
		return this.audio.duration / 3;
	}
...
Three.LineStrip = 0;
Three.LinePieces = 1;
camera_CamData.cam_Shomen = new camera_CamData("shomen",1800,0,0.15,40,200);
camera_CamData.cam_PersNaname = new camera_CamData("naname",1800,-0.7,0.4,50,100);
camera_CamData.cam_Ue = new camera_CamData("ue",820,0,1.15,85,200);
camera_CamData.cam_Yoko = new camera_CamData("yoko",1800,-1.57,0,45,0);
camera_CamData.cam_Pers = new camera_CamData("perspective",500,0.34,1.2,140,0);
camera_CamData.cam_Pers2 = new camera_CamData("perspective2",500,-2.6,1.1,140,0,true);
camera_CamData.cam_Orth1 = new camera_CamData("orth",5500,0.7,0.7,15,0);
camera_CamData.cam_Gyaku = new camera_CamData("gyaku",2000,-3.14,0.3,50,300,true);
camera_CamData.cam_Orth2 = new camera_CamData("orth3",8000,2.66,0.75,10,0,true);
camera_CamData.cam_Migi = new camera_CamData("migi",1800,0.7,0.05,60,300);
camera_CamData.cam_Yori = new camera_CamData("yori",1500,-0.37,0.13,60,300);
camera_CamData.cam_Shita = new camera_CamData("shita",1200,0.55,-0.2,55,300);
camera_CamData.cams = [camera_CamData.cam_Shomen,camera_CamData.cam_PersNaname,camera_CamData.cam_Ue,camera_CamData.cam_Shita,camera_CamData.cam_Yoko,camera_CamData.cam_Pers,camera_CamData.cam_Orth1,camera_CamData.cam_Gyaku,camera_CamData.cam_Migi,camera_CamData.cam_Yori,camera_CamData.cam_Pers2];
camera_CamData.index = -1;
...
data_ColorData.BLACK_RED = new data_ColorData({ bg : new THREE.Color(0,0,0), box : new THREE.Color(1,0,0), line : new THREE.Color(1,1,0), paper : new THREE.Color(1,1,0), moji : new THREE.Color(1,0,0)});
data_ColorData.BLACK_BLUE = new data_ColorData({ bg : new THREE.Color(0,0,0), box : new THREE.Color(0,0,1), line : new THREE.Color(1,1,0), paper : new THREE.Color(1,1,0), moji : new THREE.Color(1,0,0)});
data_ColorData.BLACK_GREEN = new data_ColorData({ bg : new THREE.Color(0,0,0), box : new THREE.Color(0,1,0), line : new THREE.Color(1,1,0), paper : new THREE.Color(1,1,0), moji : new THREE.Color(1,1,0)});
data_ColorData.RED = new data_ColorData({ bg : new THREE.Color(1,0,0), box : new THREE.Color(1,1,0), line : new THREE.Color(1,1,0), paper : new THREE.Color(1,1,0), moji : new THREE.Color(1,1,0)});
data_ColorData.GREEN = new data_ColorData({ bg : new THREE.Color(0,1,1), box : new THREE.Color(0,0,1), line : new THREE.Color(0,0,1), paper : new THREE.Color(1,1,0), moji : new THREE.Color(1,1,0)});
data_ColorData.BLUE = new data_ColorData({ bg : new THREE.Color(0,0,1), box : new THREE.Color(1,0,0), line : new THREE.Color(1,1,0), paper : new THREE.Color(1,1,0), moji : new THREE.Color(1,0,0)});
data_ColorData.CYAN = new data_ColorData({ bg : new THREE.Color(0,1,1), box : new THREE.Color(1,0,1), line : new THREE.Color(1,0,1), paper : new THREE.Color(1,1,0), moji : new THREE.Color(1,0,1)});
data_ColorData._colors = [data_ColorData.BLACK_RED,data_ColorData.BLUE,data_ColorData.BLACK_GREEN,data_ColorData.GREEN,data_ColorData.BLACK_BLUE,data_ColorData.RED];
data_ColorData._index = -1;
data_DataManager.internallyCalled = false;
data_NoteonData.BEAT = "BEAT";
data_NoteonData.VOCAL = "VOCAL";
data_Params.duration = 17.;
data_Params.distance = 9000;
...
sound_MyAudio.EVENT_500MS = "500ms";
sound_MyAudio.FFTSIZE = 64;
sound_MyAudio.LOOP_NUM = 3;
