...
data_DataManager.prototype = {
	load: function(callback) {
		this._callback = callback;
		this.cats = new objects_CatsLoader();
		this.cats.load($bind(this,this._onLoad));
	}
	,_onLoad: function() {
		console.log("==onLoad==");
		this.box = new objects_MyBoxLoader();
		this.box.load("box/box.dae",$bind(this,this._onLoad2));
	}
	,_onLoad2: function() {
		this.fontShape = new FontShapeMaker();
		this.fontShape.init("DINBold.json",$bind(this,this._onLoad3));
	}
	,_onLoad3: function() {
		this.moji = new objects_MyBoxLoader();
		this.moji.load("moji/moji.dae",$bind(this,this._onLoad4));
	}
	,_onLoad4: function() {
		if(this._callback != null) this._callback();
	}
...
var objects_CatsBase = function() {
	THREE.Object3D.call(this);
};
objects_CatsBase.__name__ = true;
objects_CatsBase.__super__ = THREE.Object3D;
objects_CatsBase.prototype = $extend(THREE.Object3D.prototype,{
	init: function(cam) {
		this._cam = cam;
	}
	,update: function(a) {
	...
	,__class__: objects_CatsBase
});
var objects_CatsGenerator = function() {
	this._showing = false;
	this._active = false;
	THREE.Object3D.call(this);
};
objects_CatsGenerator.__name__ = true;
objects_CatsGenerator.__super__ = THREE.Object3D;
objects_CatsGenerator.prototype = $extend(THREE.Object3D.prototype,{
	init: function(index) {
		this._cats = [];
		this._index = index;
		this.ground = new objects_Ground();
		this.ground.init();
	...
	,gen: function(value) {
		this._activeCat = new objects_Cat();
	...
	,__class__: objects_CatsGenerator
});
var objects_CatsGenerators = function() {
	this._count = 0;
	this.numY = 6;
	this.numX = 6;
	objects_CatsBase.call(this);
};
objects_CatsGenerators.__name__ = true;
objects_CatsGenerators.__super__ = objects_CatsBase;
objects_CatsGenerators.prototype = $extend(objects_CatsBase.prototype,{
	init: function(cam) {
		this._cam = cam;
		this._generators = [];
		var count = 0;
		...
		while(_g1 < _g) {
			...
			while(_g3 < _g2) {
				var j = _g3++;
				var gen = new objects_CatsGenerator();
				if(count > 31) count = 31;
				gen.init(count);
				gen.rotation.x = Math.random() * 2 * Math.PI;
				gen.rotation.y = Math.random() * 2 * Math.PI;
				gen.rotation.z = Math.random() * 2 * Math.PI;
				gen.position.x = i * 200 - 200 * this.numX / 2 + 100.;
				gen.position.z = j * 200 - 200 * this.numY / 2 + 100.;
				this.add(gen);
	...
	,gen: function() {
		var gen = this._generators[Math.floor(this._generators.length * Math.random())];
		gen.gen(Math.random());
	}
	,reposition: function(nx,ny) {
		...
		while(_g2 < nx) {
			...
			while(_g11 < ny) {
				...
				gen.visible = true;
				gen.position.x = i1 * 200 - 200 * nx / 2 + 100.;
				gen.position.z = j * 200 - 200 * ny / 2 + 100.;
				cnt++;
			}
		}
	}
	...
	,__class__: objects_CatsGenerators
});
var objects_CatsLoader = function() {
	THREE.Object3D.call(this);
};
objects_CatsLoader.__name__ = true;
objects_CatsLoader.__super__ = THREE.Object3D;
objects_CatsLoader.prototype = $extend(THREE.Object3D.prototype,{
	load: function(callback) {
		this._callback = callback;
		this.head = new objects_MyCATLoader();
		this.hip = new objects_MyCATLoader();
		this.body = new objects_MyCATLoader();
		this.circle = new objects_MyCATLoader();
		this.tail = new objects_MyCATLoader();
	...
	,__class__: objects_CatsLoader
});
var objects_Ground = function() {
	this.light = 1;
	THREE.Object3D.call(this);
};
objects_Ground.__name__ = true;
objects_Ground.__super__ = THREE.Object3D;
objects_Ground.prototype = $extend(THREE.Object3D.prototype,{
	init: function() {
		var ww = 100.;
		if(objects_Ground.geo == null) {
			objects_Ground.geo = new THREE.BoxGeometry(ww,ww,ww,1,1,1);
			objects_Ground.mate = new THREE.MeshLambertMaterial({ color : 34816, side : 2});
			objects_Ground.lineGeo = new THREE.Geometry();
			var shape = new THREE.Shape();
			var _g = 0;
			while(_g < 20) {
				var i = _g++;
				objects_Ground.lineGeo.vertices.push(new THREE.Vector3(ww / 2 * Math.cos(i / 19 * 2 * Math.PI),0,ww / 2 * Math.sin(i / 19 * 2 * Math.PI)));
			...
			objects_Ground.shapeGeo = new THREE.ShapeGeometry(shape);
		}
		this.circleMat = new THREE.MeshBasicMaterial({ color : 0});
		this.circle = new THREE.Mesh(objects_Ground.shapeGeo,this.circleMat);
		this.circle.rotation.x = -Math.PI / 2;
		this.circle.position.y = ww / 2 + 1;
		this.add(this.circle);
		this.line = new THREE.Line(objects_Ground.lineGeo,new THREE.LineBasicMaterial({ color : 14527027, linewidth : 2}));
		this.add(this.line);
		this.line.position.y = ww / 2 + 2;
		var m = new THREE.Mesh(objects_Ground.geo,objects_Ground.mate);
		this.add(m);
...
objects_LongCats.__name__ = true;
objects_LongCats.__super__ = objects_CatsBase;
objects_LongCats.prototype = $extend(objects_CatsBase.prototype,{
	init: function(cam) {
		this._cam = cam;
		this._cats = [];
		this._waku = new objects_LongCatsWaku();
		this._waku.init();
		this.add(this._waku);
		var _g = 0;
		while(_g < 4) {
	...
	,__class__: objects_LongCats
});
var objects_LongCatsWaku = function() {
	THREE.Object3D.call(this);
};
objects_LongCatsWaku.__name__ = true;
objects_LongCatsWaku.__super__ = THREE.Object3D;
objects_LongCatsWaku.prototype = $extend(THREE.Object3D.prototype,{
	init: function() {
	...
	,__class__: objects_LongCatsWaku
});
var objects_MyDAELoader = function() {
};
objects_MyDAELoader.__name__ = true;
objects_MyDAELoader.getPosY = function(ratio) {
	ratio = 1 - ratio;
	var maxY = 1.36578;
	var minY = -1.13318;
	return minY + (maxY - minY) * ratio;
};
objects_MyDAELoader.getRatioY = function(posY,pow,posR) {
	var maxY = 1.36578;
	var minY = -1.13318;
...
objects_MyDAELoader.getHeight = function(ratio) {
	return 2.4989600000000003 * ratio;
};
objects_MyDAELoader.prototype = {
	...
	,__class__: objects_MyDAELoader
};
var objects_MyBoxLoader = function() {
	this.scale = 100;
	objects_MyDAELoader.call(this);
};
objects_MyBoxLoader.__name__ = true;
objects_MyBoxLoader.__super__ = objects_MyDAELoader;
objects_MyBoxLoader.prototype = $extend(objects_MyDAELoader.prototype,{
	load: function(filename,callback) {
		this._callback = callback;
		var loader = new THREE.ColladaLoader();
		loader.options.convertUpAxis = true;
	...
	,_onComplete: function(ret) {
		...
		;
		console.info("____");
		console.info(this.geos[0]);
		if(this._callback != null) this._callback();
	}
	,__class__: objects_MyBoxLoader
});
var objects_MyCATLoader = function() {
	this.scale = 100;
	objects_MyDAELoader.call(this);
};
objects_MyCATLoader.__name__ = true;
objects_MyCATLoader.__super__ = objects_MyDAELoader;
objects_MyCATLoader.prototype = $extend(objects_MyDAELoader.prototype,{
	load: function(filename,callback) {
		this._callback = callback;
		var loader = new THREE.ColladaLoader();
		loader.options.convertUpAxis = true;
	...
	,_onComplete: function(ret) {
		var dae = ret.scene;
		var mm = new shaders_MaeShaderMaterial("./cat/cat_diff.jpg");
		this.mesh = null;
	...
	,clone: function() {
		if(this.geometry == null) console.error("error geo");
		if(this.material == null) console.error("error mat");
		var m = new THREE.Mesh(this.geometry,this.material);
		m.scale.set(this.scale,this.scale,this.scale);
		return m;
	}
	,__class__: objects_MyCATLoader
});
...
objects_XmasGenerator.prototype = $extend(THREE.Object3D.prototype,{
	...
	,addObject: function(d,pos) {
		console.info("_count" + this._count);
		if(this._count >= 54) {
...
var objects_bg_BgSphere = function() {
	...
	console.log("len = " + Std.string(g.attributes.position.length));
...
objects_CatsGenerators.SPACE = 200;
